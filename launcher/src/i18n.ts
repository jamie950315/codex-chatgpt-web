...
const en = {
  ...
  biggerContextBody: "Experimental: small requests stay inline. Pro sends large original context as one TXT attachment; file contents, instructions and images all count toward the context limit. Other modes retain multipart delivery. Upload failures are reported without silently trimming context or switching models. Restart Codex after changing this setting. Uploads and extra requests may increase waiting time or cooldowns. Disabled by default.",
  biggerContextRecommendationTitle: "Use a larger ChatGPT Web context",
  biggerContextRecommendationBody: "Bigger Context uses a full-context TXT attachment for large Pro requests and multipart delivery for other modes. It remains experimental and may increase waiting time or cooldowns.",
  biggerContextRecommendationToggleBody: "Use larger context with automatic Pro TXT delivery or multipart delivery on other modes.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "实验性：小请求仍直接发送；Pro 的大型原始上下文会作为一个 TXT 附件发送，文件、指令和图片都会计入上下文限制。其他模式保留分段发送。上传失败会报错，不会静默删减内容或切换模型。更改后请重启 Codex。上传和额外请求可能增加等待或冷却时间。默认关闭。",
  biggerContextRecommendationTitle: "使用更大的 ChatGPT Web 上下文",
  biggerContextRecommendationBody: "Bigger Context 会将 Pro 的大型原始上下文作为 TXT 附件发送；其他模式使用分段发送。仍属实验性，可能增加等待或冷却时间。",
  biggerContextRecommendationToggleBody: "使用更大上下文：Pro 自动上传 TXT，其他模式使用分段发送。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "試験的機能：小さなリクエストは直接送信し、Pro の大きな元コンテキストは TXT 添付で送信します。ファイル、指示、画像すべてを上限に計上します。他のモードは分割送信を維持します。アップロード失敗時に内容を黙って削除したりモデルを変更したりしません。変更後は Codex を再起動してください。待機時間やクールダウンが増える場合があります。初期設定では無効です。",
  biggerContextRecommendationTitle: "より大きな ChatGPT Web コンテキストを使用",
  biggerContextRecommendationBody: "Bigger Context は Pro の大きな元コンテキストを TXT 添付で送信し、他のモードでは分割送信します。試験的機能のため、待機時間やクールダウンが増える場合があります。",
  biggerContextRecommendationToggleBody: "大きなコンテキストを使用：Pro は自動 TXT 添付、他のモードは分割送信。",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "實驗性：小請求仍直接傳送；Pro 的大型原始上下文會作為一個 TXT 附件傳送，檔案、指令和圖片都會計入上下文限制。其他模式保留分段傳送。上傳失敗會報錯，不會悄悄刪減內容或切換模型。變更後請重新啟動 Codex。上傳和額外請求可能增加等待或冷卻時間。預設關閉。",
  biggerContextRecommendationTitle: "使用更大的 ChatGPT Web 上下文",
  biggerContextRecommendationBody: "Bigger Context 會將 Pro 的大型原始上下文作為 TXT 附件傳送；其他模式使用分段傳送。仍屬實驗性，可能增加等待或冷卻時間。",
  biggerContextRecommendationToggleBody: "使用更大上下文：Pro 自動上傳 TXT，其他模式使用分段傳送。",
