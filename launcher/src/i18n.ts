...
const en = {
  ...
  biggerContextBody: "Warning: keeps small turns as one message, then splits large context across two to eight messages. Oversized individual records are split so each ChatGPT composer message still fits. The final part starts the task without an extra request. It raises the model context and compaction thresholds; restart Codex after changing it. Extra requests may increase rate limits or temporary cooldowns. Disabled by default.",
  biggerContextRecommendationTitle: "Use a larger ChatGPT Web context",
  biggerContextRecommendationBody: "Bigger Context sends large tasks in multiple messages so ChatGPT Web can carry more context. It remains experimental and can increase rate limits or temporary cooldowns.",
...
const zh: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "警告：较小回合仍作为一条消息发送；较大的上下文会拆分为两条到八条消息。超过单则上限的记录会被切开，好让每条 ChatGPT 输入仍能装下。最后一部分会直接开始任务，不会额外发送请求。模型上下文和压缩阈值会提高；更改后请重启 Codex。额外请求可能提高触发速率限制或临时冷却的概率。默认关闭。",
  biggerContextRecommendationTitle: "使用更大的 ChatGPT Web 上下文",
  biggerContextRecommendationBody: "Bigger Context 通过多条消息发送大型任务，让 ChatGPT Web 承载更多上下文。此功能仍处于实验阶段，可能增加触发速率限制或临时冷却的概率。",
...
const ja: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "警告：小さなターンは 1 件のメッセージとして維持し、大きなコンテキストは 2～8 件のメッセージに分割します。単一レコードが上限を超える場合は断片化して、各 ChatGPT 入力が収まるようにします。最後の部分から追加リクエストなしでタスクを開始します。モデルのコンテキスト上限とコンパクションしきい値が上がります。変更後は Codex を再起動してください。追加リクエストにより、レート制限や一時的なクールダウンが発生しやすくなる場合があります。初期設定では無効です。",
  biggerContextRecommendationTitle: "より大きな ChatGPT Web コンテキストを使用",
  biggerContextRecommendationBody: "Bigger Context は大きなタスクを複数のメッセージで送信し、ChatGPT Web でより多くのコンテキストを運べるようにします。試験的な機能であり、レート制限や一時的なクールダウンが発生しやすくなる場合があります。",
...
const zhTW: Record<keyof typeof en, string> = {
  ...
  biggerContextBody: "警告：較小回合仍作為一則訊息傳送；較大的上下文會拆成兩則到八則訊息。超過單則上限的記錄會被切開，好讓每則 ChatGPT 輸入仍能裝下。最後一部分會直接開始任務，不會額外傳送請求。模型上下文與壓縮閾值會提高；變更後請重新啟動 Codex。額外請求可能提高觸發速率限制或暫時冷卻的機率。預設關閉。",
  biggerContextRecommendationTitle: "使用更大的 ChatGPT Web 上下文",
  biggerContextRecommendationBody: "Bigger Context 透過多則訊息傳送大型任務，讓 ChatGPT Web 承載更多上下文。此功能仍處於實驗階段，可能增加觸發速率限制或暫時冷卻的機率。",
  biggerContextRecommendationToggleBody: "將大型任務分成多則訊息，並提高模型上下文與壓縮限制。",
