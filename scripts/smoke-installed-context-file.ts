...
import { allowWebOnlySmokeRequest } from "./web-only-request-guard";
...
const pro = catalog.models?.find((model: any) => model.slug === "chatgpt-web/pro");
if (!pro) throw new Error("Installed WebGPT has no Pro row");
writeFileSync(join(catalogRoot, "models.json"), JSON.stringify({ models: [pro] }), { mode: 0o600 });
...
const guardian = Bun.serve({ hostname: "127.0.0.1", port: 0, idleTimeout: 0, async fetch(request) {
  ...
  if (request.method === "GET" && path === "/v1/models") return Response.json({ models: [pro] });
  ...
  const accepted = allowWebOnlySmokeRequest(request.method, path, body.model);
  ...
  if (!accepted) return Response.json({ error: { message: "Only ChatGPT Web Pro is permitted by this test", type: "invalid_request_error" } }, { status: 403 });
...
async function run(label: string, prompt: string, thread?: string) {
  const args = ["exec", ...clientConfig, ...(thread ? ["resume", thread] : ["--sandbox", "read-only", "--skip-git-repo-check"]),
    "--json", "--model", "chatgpt-web/pro", "--output-last-message", join(root, `${label}-answer.txt`), "-"];
...
console.log(JSON.stringify({ event: "INSTALLED_FILE_SMOKE_START", root, port: guardian.port }));
try {
  // Dense text exercises ~300k total tokens without exceeding Codex's single-input character cap.
  const thread = await run("first", `這是僅限 ChatGPT Web Pro 的驗證。禁止切換模型、建立子代理、改檔、提交或重啟服務。保留最早與最後的兩個識別值。\n${head}\n`
  ...
  writeFileSync(join(root, "success.json"), JSON.stringify({ passed: true, thread, requests: requests.length }), { mode: 0o600 });
