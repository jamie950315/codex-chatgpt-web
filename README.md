...
With **Bigger Context** enabled, the automatic **Pro** mode now uploads large canonical task
context as one TXT attachment instead of splitting it across older chat messages. Small Pro
requests remain inline; other modes retain their existing transport, and no model is switched
automatically. Each new file-based Pro turn opens a fresh web conversation and uploads the full
